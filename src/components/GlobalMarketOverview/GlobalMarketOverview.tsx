  ...
  return (
    <Card className={className}>
      ...
      <CardContent className='flex flex-wrap gap-24 justify-center'>
        ...
        {!isLoading ? (
          <>
            <MiniCard isCurrency value={data?.totalMarketCap} heading='Total market cap' percentage={`${data?.marketCapChange.toFixed(2)} %`} />
            <MiniCard isCurrency value={data?.totalVolume} heading='24h Volume' percentage='-' />
            <MiniCard value={data?.activeCryptos} heading='Total cryptocurrencies' />
            <MiniCard value={`${data?.bitcoinDominance.toFixed(1)}%`} heading='Bitcoin dominance' />
          </>
        ) : (

          <Skeleton className='w-200 h-100' />
        )}
